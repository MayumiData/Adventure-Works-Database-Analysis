...
SELECT DimProduct.EnglishProductName, SUM(FactProductInventory.UnitsBalance) AS
UnitsBalance
FROM dbo.FactProductInventory AS FactProductInventory
INNER JOIN dbo.DimProduct AS DimProduct
...
SELECT DimSalesTerritory.SalesTerritoryCountry, SUM(FactInternetSales.SalesAmount) AS
SalesAmount
FROM dbo.DimSalesTerritory AS DimSalesTerritory
INNER JOIN dbo.FactInternetSales AS FactInternetSales
...
--3. Create a stored procedure that runs the above created SQL statement dynamically so that
the user specifies the Sales country.
CREATE PROCEDURE SaleCountry
...
INNER JOIN dbo.FactInternetSales AS FactInternetSales
ON DimSalesTerritory.SalesTerritoryKey = FactInternetSales.SalesTerritoryKey
WHERE DimSalesTerritory.SalesTerritoryCountry = @SalesTerritoryCountry
GROUP BY DimSalesTerritory.SalesTerritoryCountry
END
GO
...
--5. Create another stored procedure that will create a table whenever it runs. The table will be
CustomerGeography.
CREATE PROCEDURE CustomerGeography
AS
BEGIN
DECLARE @RowCount int
SELECT DimCustomer.*, DimGeography.City, DimGeography.EnglishCountryRegionName
FROM dbo.DimCustomer AS DimCustomer
INNER JOIN dbo.DimGeography AS DimGeography
ON DimCustomer.GeographyKey = DimGeography.GeographyKey
SET @RowCount = @@ROWCOUNT
INSERT INTO MyLogTable (Username, ExecutionTime, row_count)
VALUES (CURRENT_USER, GETDATE(), @RowCount)
END
GO
EXEC CustomerGeography


-- 6. Create a log table that has log_date_time, row_count as columns and use this table to log
the above stored procedures executions.
...
CREATE PROCEDURE GetAverageProductSalesAmountUSD
AS
BEGIN
SELECT
...
ROUND(AVG(FactInternetSales.SalesAmount * CurrencyAvg.EndOfDayRate), 2) AS
AverageSalesAmountUSD
FROM
...
WHERE DateKey BETWEEN 20120101 and 20131231
GROUP BY CurrencyKey, DateKey
) AS CurrencyAvg
...
ORDER BY
FactInternetSales.OrderDateKey
INSERT INTO MyLogTable_2 (Username, ExecutionTime)
